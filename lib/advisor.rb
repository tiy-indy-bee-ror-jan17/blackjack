...
module Advisor
  ...
  def stand?(hand)
    hand.value >= 17 ||
      (hand.value.between?(12, 16) &&
       dealer_hand.show_card.value.between?(2, 6))
  end

  def soft_hit?(hand)
    (hand.soft? && hand.value <= 17) ||
      (hand.soft? && hand.value == 18 &&
       dealer_hand.show_card.value.between?(8, 10))
  end
  ...
  def split_twos_threes?(hand)
    (all_card?(hand, '2') ||
     all_card?(hand, '3')) &&
      dealer_hand.show_card.value.between?(4, 7)
  end

  def split_sixes?(hand)
    all_card?(hand, '6') && dealer_hand.show_card.value.between?(3, 6)
  end

  def split_sevens?(hand)
    all_card?(hand, '7') && dealer_hand.show_card.value.between?(3, 7)
  end
